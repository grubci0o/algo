...
class BST{
    ...
    void postOrder(BST*);
    void printPaths(BST*);
    void printPathsRecurr(BST*,int[],int);
...
void BST::printPaths(BST*root){
    int patharr[100];
    printPathsRecurr(root,patharr,0);
    
    
}
void BST::printPathsRecurr(BST*root,int path[],int pathlen){
    if(!root){
        return;
    }
    ...
    if(root->left == nullptr && root->right == nullptr){
        ...
        printPathsRecurr(root->right,path,pathlen);
        printPathsRecurr(root->left,path,pathlen);
        
    }
    
}
void BST::printArray(int ints[], int len)
{
    int i;
    for (i = 0; i < len; i++)
    {
